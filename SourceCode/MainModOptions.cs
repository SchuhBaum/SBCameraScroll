...
public class MainModOptions : OptionInterface
{
    ...
    public static Configurable<bool> regionMods = instance.config.Bind("regionMods", defaultValue: true, new ConfigurableInfo("When enabled, and region mods are enabled or disabled the corresponding cached room textures get merged again. The load order matters if multiple mods change the same room.", null, "", "Region Mods"));
